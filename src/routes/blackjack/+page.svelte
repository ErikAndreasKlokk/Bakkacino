...
const cards = [
    ...
    let PlayerSplitValue = []

    let SumPlayerValue = 0
    let SumPlayerSplitValue = 0
    let SumDealerValue = 0

    let DealerHvilketKort = 1
    let HvilketKort = 2
    let HvilketKortSplit = 2
...
function carddraw(){
    ...
    let PlayerSplitBrett = document.getElementById('PlayerSplitBrett')

    SpillerKort.innerHTML = ""
    DealerKort.innerHTML = ""
    PlayerSplitBrett.innerHTML = ""
    ...
    let Kort1 = PlayerValue[0]  
    let Kort2 = PlayerValue[1]
...
function PlayerSplit(){
    ...
    Card = cards[RandomCard].link
    CardValue = cards[RandomCard].value
    PlayerSplitCards = [...PlayerSplitCards, Card]
    PlayerSplitValue = [...PlayerSplitValue, CardValue] 
    let PlayerHvorAce = PlayerValue.indexOf(11) 
    SumPlayerSplitValue = 0
    for (let j = 0; j < PlayerSplitValue.length; j++) {
            SumPlayerSplitValue += PlayerSplitValue[j];
            }
    if (SumPlayerSplitValue > 21){
        if (PlayerSplitValue.includes(11)){
            PlayerSplitValue[PlayerHvorAce] = 1
            SumPlayerSplitValue = 0
            for (let j = 0; j < PlayerSplitValue.length; j++) {
            SumPlayerSplitValue += PlayerSplitValue[j];
            }
            ...
            WinLost = "du tapte første brett"
            console.log("player loose brett 1")
    }
    }


    console.log('Current HvorSkalKortet:', HvorSkalKortetSplit)
    let PlayerSplitBrett = document.getElementById('PlayerSplitBrett')
    let nyttkort = document.createElement('img')
    nyttkort.src = PlayerSplitCards[HvilketKortSplit]
    nyttkort.classList.add('h-[100%]', 'absolute', 'z-10')
    nyttkort.style.left = HvorSkalKortetSplit + 'rem'
    PlayerSplitBrett.appendChild(nyttkort);
    HvorSkalKortetSplit = HvorSkalKortetSplit + 5
    HvilketKortSplit = HvilketKortSplit + 1
}
function MakeImage(){
    ...
    <div class="flex flex-col items-center justify-center h-screen w-[calc(100%-20.625rem)]">
        ...
        <div class="flex items-center justify-between bg-primary-700 h-full w-full max-w-[1200px] max-h-[850px]" in:fade={{duration: 1000}}>
            ...
            <div id="spillbrett" class="flex flex-col items-center justify-between bg-primary-900 h-[92%] w-[58%] mr-[2.1%]">
                ...
                <div id="PlayerSpillbrett" class="flex relative h-[11rem] w-[100%]">
                    <div id="PlayerSplitBrett" class="flex relative border-[0.1rem] mb-[1rem] border-black h-[11rem] w-[28%]">
